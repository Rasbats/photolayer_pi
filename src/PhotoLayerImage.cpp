 ...
 *
 ***************************************************************************
 *   Copyright (C) 2017 Mike Rossiter                                      *
 *                                                                         *
...
PhotoLayerImageCoordinates::MapType PhotoLayerImageCoordinates::GetMapType(wxString name)
{
    for(int i=0; i < MAP_TYPES; i++) {
        PhotoLayerImageCoordinates::MapType type = (PhotoLayerImageCoordinates::MapType)i;
...
/* page 1 */
void PhotoLayerImage::MakePhasedImage()
{
    if(!m_origimg.IsOk()) {
...
//    m_phasedimg = wxImage(m_phasedimg.GetWidth(), m_phasedimg.GetHeight()-1, m_phasedimg.GetData(), true);
    ...
    case PhotoLayerImageCoordinates::CCW:
        m_phasedimg = m_phasedimg.Rotate90(false);
        break;
    case PhotoLayerImageCoordinates::R180:
        m_phasedimg = m_phasedimg.Rotate90(true);
    case PhotoLayerImageCoordinates::CW:
        m_phasedimg = m_phasedimg.Rotate90(true);
    default:
        break;
    }
...
void PhotoLayerImage::InputToMercator(double px, double py, double &mx, double &my)
{
    double dx, dy;
    ...
    case PhotoLayerImageCoordinates::MERCATOR:
        ...
        y = dy;
        break;
    case PhotoLayerImageCoordinates::POLAR:
    case PhotoLayerImageCoordinates::CONIC:
    {
    ...
    case PhotoLayerImageCoordinates::FIXED_FLAT:
        x = dx;
        pp = dy / inputheight - 1;
        break;
    default: break;
...
void PhotoLayerImage::MercatorToInput(double mx, double my, double &px, double &py)
{
    double x, y;
...
bool PhotoLayerImage::MakeMappedImage(wxWindow *parent, bool paramsonly)
{
    if(!m_phasedimg.IsOk()) {
    ...
    if(m_Coords->mappingmultiplier == 1 &&
       ...
       m_Coords->mapping == PhotoLayerImageCoordinates::MERCATOR) {
...
Try changing size parameter to a smaller value. (less than %.2f)\naborting\n"),
    ...
    wxProgressDialog progressdialog(
        _("Mapping PhotoLayer Image"), _("PhotoLayer Mapper"), mw, parent,
...
bool PhotoLayerImage::GetOverlayCoords(PlugIn_ViewPort *vp, wxPoint p[3], int &w, int &h)
{
    if(!m_Coords)
        return false;
...
void PhotoLayerImage::RenderImage(wxDC &dc, PlugIn_ViewPort *vp)
{
    wxPoint p[4];
    int w, h;
    if(vp->rotation)
...
void PhotoLayerImage::RenderImageGL(PlugIn_ViewPort *vp)
{
